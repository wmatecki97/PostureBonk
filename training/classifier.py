...
def black_and_white_images_generator(generator):
    for x, y in generator:
        x = [convert_to_black_and_white(img) for img in x]
        x = np.array(x)
        yield x, y
...
train_generator = black_and_white_images_generator(train_datagen.flow_from_directory(
...
model.fit(train_generator, epochs=epochs,
          steps_per_epoch=steps_per_epoch, callbacks=[lr_scheduler])

# Save the model to a file
model.save(os.path.join(os.path.dirname(os.path.abspath(__file__)),
           '..', 'app', 'assets', 'image_classifier_model.h5'))
